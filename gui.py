...
def _write_config(data: dict) -> None:
    """Write config.py from the wizard data dict (mirrors setup.py logic)."""
    children = [c["name"] for c in data.get("children", [])]
    text = f'''\
"""
config.py – Settings for the Storypark Photo Pipeline.
Generated by the setup wizard.  Re-run setup.py or open Settings to change.
"""

# Filled in by the setup wizard.  Change via Settings, not by hand.
STORYPARK_EMAIL = {data["storypark_email"]!r}
...
def get_storypark_password() -> str:
    try:
        ...
        raise RuntimeError(f"Could not access keychain: {{exc}}") from exc
    raise RuntimeError(
        "Storypark password not found.  Re-run the setup wizard."
    )


CHILDREN = {children!r}
REFERENCE_ENCODINGS_FILE = "face_encodings.pkl"
DAYCARE_LATITUDE  = {data["latitude"]!r}
DAYCARE_LONGITUDE = {data["longitude"]!r}
TEMP_DIR = "tmp_photos"
STATE_DB_PATH = "processed_posts.db"
GOOGLE_CREDENTIALS_FILE = "client_secret.json"
GOOGLE_TOKEN_FILE = "token.json"
HEADLESS_BROWSER = {data["headless"]!r}
MAX_POSTS = {data["max_posts"]!r}
INCREMENTAL_STOP_THRESHOLD = 5
'''
    with open(APP_DIR / "config.py", "w", encoding="utf-8") as fh:
        fh.write(text)
