...
Everything is saved to config.py and face_encodings.pkl so you never need to
edit config files or provide reference photos manually.
"""
...
def _write_config(cfg: dict) -> None:
    """Regenerate config.py from the collected wizard values."""

    children_repr = repr(cfg["children"])
    config_text = f'''\
"""
config.py – Settings for the Storypark Photo Pipeline.

Generated by setup.py – re-run setup.py to change any of these values.
"""

# ---------------------------------------------------------------------------
# Storypark credentials
# ---------------------------------------------------------------------------
STORYPARK_EMAIL = {cfg["storypark_email"]!r}
STORYPARK_PASSWORD = {cfg["storypark_password"]!r}
...
# ---------------------------------------------------------------------------
DAYCARE_LATITUDE = {cfg["latitude"]!r}
DAYCARE_LONGITUDE = {cfg["longitude"]!r}
...
def run_wizard() -> None:
    ...
    cfg["storypark_password"] = _ask_password("Storypark password")
    ...
    try:
        from config import REFERENCE_ENCODINGS_FILE  # noqa: PLC0415
    except ImportError:
        REFERENCE_ENCODINGS_FILE = "face_encodings.pkl"
    ...
    print()
    print("=" * 62)
    print("  ✓ Setup complete!  Configuration saved to config.py")
    print()
    print("  Next steps:")
